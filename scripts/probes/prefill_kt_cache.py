...
print(
    ...
    f"KV={KV} QPK={QPK} D={D} iters={ITERS} arms={ARMS}"
)
...
tables = [
    torch.tensor([pages_used[i]], dtype=torch.int32).to("spyre") for i in range(NUM_BLOCKS)
]
...
for arm in ARMS:
    transposed = arm == "kt"
    k_dev, v_dev = build(transposed)
    fn = torch.compile(kernel, dynamic=False)
    args = (query_dev, row_index, k_dev, v_dev, tables, masks, SCALE, NUM_BLOCKS, Q_LEN,
...
if "kt" in results and "baseline" in results:
    d = results["kt"] - results["baseline"]
    print(
        f"\nKT-CACHE {results['baseline']:.1f} -> {results['kt']:.1f}us "
        f"({100.0 * d / results['baseline']:+.1f}%)"
    )
