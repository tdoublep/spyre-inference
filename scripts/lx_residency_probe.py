...
def build_kernel(variant: str, cfg: dict):
    ...
    if variant == "transposed-pv":
        return _transposed_pv_kernel(cfg)
    if variant == "slot-col":
        return _slot_col_kernel(cfg)
    if variant == "transposed-pv-q1":
        return _transposed_pv_q1_kernel(cfg)
    if variant == "static-slice":
        return _static_slice_kernel(cfg)
    if variant == "static-slice-hm":
        return _static_slice_hm_kernel(cfg)
    raise SystemExit(f"unknown variant {variant!r}")


def _static_slice_hm_kernel(cfg: dict):
    """static-slice + head-major V: the two fixes composed.

    The page cache carries two independent LX vetoes and each fix clears exactly
    one of them:
      - "index tensor or indirectly accessed" -> cleared by reading pages with a
        static slice rather than an indirect gather (see static-slice).
      - "core div mismatch" on the V operand tile -> cleared by making V the
        transposed operand, i.e. a head-major cache (see mixed-layout).
    Neither alone gets both lanes. This checks they compose.

    k_pages: [pages, block, kv, head]   v_pages: [pages, kv, head, block]
    """
    import torch
    ...
    def kernel(query, query_row_index, k_pages, v_pages, page_index_table, mask_tiles, scale):
        ...
        for i in range(num_blocks):
            k_page_4d = k_pages[i].permute(1, 0, 2).unsqueeze(1)
            # Already [kv, head, block]; the QPK axis is the unsqueeze.
            v_page_4d = v_pages[i].unsqueeze(1)
...
def _static_slice_kernel(cfg: dict):
    """Read pages by static slice instead of indirect gather.

    The page cache is refused LX categorically -- "index tensor or indirectly
    accessed" -- and the gather output is refused because the indirect `index` op
    carries no core division (num_cores=1) while its consumer runs 32. Neither
    veto is about axis order, which is why every cache-layout variant moves the
    operand tile and leaves the page itself spilling to HBM.

    This models the alternative: hoist the gather out of the inner loop into a
    compact per-step staging buffer, then index that buffer with a Python int in
    the unrolled loop, so every inner-loop read is direct and statically known.
    Same arithmetic as baseline. If the page tiles still miss LX here, then no
    kernel-side restructuring reaches them and the fix has to be in torch-spyre.
    """
    import torch
    ...
    def kernel(query, query_row_index, k_pages, v_pages, page_index_table, mask_tiles, scale):
        ...
        tile_max = tile_sum = tile_output = None
        for i in range(num_blocks):
            # Static integer slice: no index tensor, nothing indirect.
            k_page_4d = k_pages[i].permute(1, 0, 2).unsqueeze(1)
            v_page_4d = v_pages[i].permute(1, 0, 2).unsqueeze(1)
            mask_tile = mask_tiles[i]
...
def _transposed_pv_q1_kernel(cfg: dict):
    """out^T = V^T @ P^T, specialized to padded_query_len == 1. Cache unchanged.

    ``transposed-pv`` had the right idea and the wrong shapes: it transposed the
    rescale factor and unsqueezed the running sum to keep them aligned with a
    transposed accumulator, and that graph does not survive dxp_standalone.

    At q == 1 none of that is needed. amax/sum keepdim already give [kv, qpk, 1,
    1], which broadcasts against a [kv, qpk, head, 1] accumulator as-is, and
    transposing tile_probs from [kv, qpk, 1, block] to [kv, qpk, block, 1] is a
    reshape of the same contiguous run, not a data movement. So the only real
    change is which operand carries the transpose -- V instead of nothing -- and
    one transpose of the [.., head, 1] result on the way out.

    Decode is 511 of 512 steps in the benchmark, and the factory already
    specializes on padded_query_len, so prefill can keep the shipped kernel.
    """
    import torch

    assert cfg["padded_query_len"] == 1
    num_blocks = cfg["num_blocks"]
    num_heads = cfg["num_heads"]
    num_kv_heads = cfg["num_kv_heads"]
    head_size = cfg["head_size"]
    ...
    def kernel(query, query_row_index, k_pages, v_pages, page_index_table, mask_tiles, scale):
        ...
        q = (
            q_rows.unsqueeze(0)
            .transpose(1, 2)
            .reshape(num_kv_heads, num_queries_per_kv, 1, head_size)
        )
        tile_max = tile_sum = tile_output_t = None
        for i in range(num_blocks):
            ...
            if i == 0:
                tile_max = scores_max
                tile_probs = torch.exp(scores - tile_max)
                # [kv, 1, head, block] @ [kv, qpk, block, 1] -> [kv, qpk, head, 1]
                tile_output_t = torch.matmul(
                    v_page_4d.transpose(-2, -1), tile_probs.transpose(-2, -1)
                )
                tile_sum = tile_probs.sum(dim=-1, keepdim=True)
            else:
                new_max = torch.maximum(tile_max, scores_max)
                rescale = torch.exp(tile_max - new_max)
                tile_output_t = tile_output_t * rescale
                tile_sum = tile_sum * rescale
                tile_probs = torch.exp(scores - new_max)
                tile_output_t += torch.matmul(
                    v_page_4d.transpose(-2, -1), tile_probs.transpose(-2, -1)
                ...
                tile_sum = tile_sum + tile_probs.sum(dim=-1, keepdim=True)
                tile_max = new_max
        attn = (tile_output_t / tile_sum).transpose(-2, -1)
        attn = attn.reshape(1, num_heads, 1, head_size).transpose(1, 2)
        return attn.reshape(1, num_heads, head_size)

    return kernel


def _slot_col_kernel(cfg: dict):
    """K token-major, V as [kv, head, pages, block] -- head-major *and* storable.

    ``mixed-layout`` proves the head-major V tile gets LX, but [pages, kv, head,
    block] puts the token axis innermost, so the slot-major store view that
    ``kv_slot_views`` builds no longer exists and the scatter has nowhere to go.

    Ordering the cache [kv, head, pages, block] keeps the same head-major tile
    while making the flat index (h*D + d) * (P*B) + (p*B + off): the trailing
    term is exactly the slot index, so a [kv*head, pages*block] view takes the
    whole scatter as one ``index_copy_`` along dim 1. The page gather moves from
    dim 0 to dim 2; this probes whether LX cares about that.

    k_pages: [pages, block, kv, head]  (unchanged)
    v_pages: [kv, head, pages, block]
    """
    import torch
    ...
    def kernel(query, query_row_index, k_pages, v_pages, page_index_table, mask_tiles, scale):
        ...
        for i in range(num_blocks):
            ...
            k_page = k_pages.index_select(0, page_idx)
            k_page_4d = k_page.squeeze(0).permute(1, 0, 2).unsqueeze(1)
            # [kv, head, 1, block] -> [kv, 1, head, block]; the QPK axis replaces
            # the gathered page axis, so this is a reshape, not a permute.
...
def _transposed_pv_kernel(cfg: dict):
    """Compute the P@V product transposed, so V is the transposed operand.

    out^T = V^T @ P^T instead of out = P @ V. The cache stays token-major, so
    nothing about the store changes -- the V tile reaches its matmul through
    ``.transpose(-2,-1)`` purely by reassociating the product. Tests whether the
    LX rule cares that an operand is transposed, or that it is the *right-hand*
    transposed operand as in the Q@K^T lane.

    The running max/sum accumulators move to the transposed orientation with the
    output; softmax is unchanged arithmetic.
    """
    import torch
    ...
    def kernel(query, query_row_index, k_pages, v_pages, page_index_table, mask_tiles, scale):
        ...
        tile_max = tile_sum_t = tile_output_t = None
        for i in range(num_blocks):
            page_idx = page_index_table[i, 0:1]
            k_page = k_pages.index_select(0, page_idx)
            v_page = v_pages.index_select(0, page_idx)
            ...
            if i == 0:
                tile_max = scores_max
                tile_probs = torch.exp(scores - tile_max)
                # [kv, 1, head, block] @ [kv, qpk, block, q] -> [kv, qpk, head, q]
                tile_output_t = torch.matmul(
                    v_page_4d.transpose(-2, -1), tile_probs.transpose(-2, -1)
                )
                tile_sum_t = tile_probs.sum(dim=-1).unsqueeze(-2)
            else:
                new_max = torch.maximum(tile_max, scores_max)
                rescale_t = torch.exp(tile_max - new_max).transpose(-2, -1)
                tile_output_t = tile_output_t * rescale_t
                tile_sum_t = tile_sum_t * rescale_t
                tile_probs = torch.exp(scores - new_max)
                tile_output_t += torch.matmul(
                    v_page_4d.transpose(-2, -1), tile_probs.transpose(-2, -1)
                )
                tile_sum_t = tile_sum_t + tile_probs.sum(dim=-1).unsqueeze(-2)
                tile_max = new_max
        attn = (tile_output_t / tile_sum_t).transpose(-2, -1)
        attn = attn.reshape(1, num_heads, padded_query_len, head_size).transpose(1, 2)
        return attn.reshape(padded_query_len, num_heads, head_size)
...
def run_standalone(args, cfg: dict, out_path: str) -> None:
    ...
    # slot-col wants V ordered [kv, head, pages, block] instead.
    if args.variant == "slot-col":
        v_dev_cpu = v_cpu.permute(2, 3, 0, 1).contiguous()
    if args.variant == "static-slice-hm":
        v_dev_cpu = v_cpu.permute(0, 2, 3, 1).contiguous()
