...
def unreachable_reason(query_lens) -> str:
    ...
    Checked against the platform cap, not the derived limits: those come from the
    shapes themselves, so they can never rule one out.
    """
...
def record_padding(row, attn_metadata, query_lens, seq_lens, block_size):
    """Record the shape the kernel got, and flag it when that is not the shape asked for.

    ``derive_lattice`` makes the builder's round-up the identity, so a mismatch here
    means the lattice and the shape list disagree and the row is measuring something
...
def run_config(entry, variant, cfg, records, csv_path, block_size=None):
    ...
    row = {
        ...
        # Realized, from the built metadata: the declared lengths are what the
        # builder was asked for, these are what the kernel iterates.
...
def derive_lattice(entries):
    ...
    Every declared length becomes its own bucket, so ``find_*_bucket`` returns it
    unchanged and the kernel runs the shape the config names. To measure what a
    deployment's coarser ladder does to a length, declare the padded length.

    Block-size independent, which a ``block_sizes`` sweep needs: the kv ladder is
    in tokens and ``num_blocks_buckets`` derives from it through the same
    ``ceil(len / block_size)`` the builder applies, so the identity survives at
    any block size. It has to be, since ``envs`` caches on first read and these
    reach the bucketer as env vars.
    """
    kv = sorted({s for e in entries for s in e["seq_lens"]})
    max_batched = max(sum(e["query_lens"]) for e in entries)
    # Each ladder's top has to be its limit, or _resolve_buckets appends it with a
    # warning. max_batched is the only one that is not already a declared length: a
    # decode-only config never asks for a query that wide.
