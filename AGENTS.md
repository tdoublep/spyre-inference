...
- **dtype**: float16 — `check_and_update_config` sets it for every model, gemma-4's vision tower included. bfloat16 is declared supported by the attention backend and is the same 2 bytes on device, but nothing selects it, so `all_reduce` and the FP8 kernel being fp16-only costs nothing today.
- **Tensor parallelism**: TP≥1 supported; `all_reduce` is provided natively by `libspyre_comms` (no longer overridden in `SpyreCommunicator`). **DP>1 is rejected** in `TorchSpyrePlatform.check_and_update_config` — the spyre-comms global rank space isn't validated for DP×TP. **bfloat16 + TP>1 is rejected there too**: torch-spyre's `all_reduce` is fp16-only on both paths (eager `SpyreCCLBackend` refuses bf16; the compiled `spyre.allreduce_plan` lowering has no bf16 `add`), so a bf16 model runs at TP1 only. Nothing selects bf16 today, so this only bites an explicit `--dtype bfloat16`. Separately, `hf_overrides=GEMMA4_TEXT_BACKBONE_OVERRIDE` (`models/gemma4.py`) loads a gemma-4 vision repo's decoder alone, skipping the tower for a text-only run.
- **Compilation**: default is compiled (`STOCK_TORCH_COMPILE`), not eager. Attention compiles in its own domain: `SpyreAttentionImpl` reads `get_current_vllm_config().compilation_config.mode` at construction, so building it outside a `set_current_vllm_config` context raises. Tests force `enforce_eager=True` where they want eager — don't assume "eager in tests." `STOCK_TORCH_COMPILE` compiles one transformer block at a time; `SPYRE_COMPILE_GRANULARITY=model` restores the whole-model graph. Attention kernels are recorded during warmup rather than compiled lazily in the serving path: warmup walks `SpyreAttnBucketer.variants()` and traces each one. `SPYRE_ATTN_RECORD=0` restores the lazy behaviour, and `SPYRE_ATTN_KV_BUCKETS` / `SPYRE_ATTN_QUERY_BUCKETS` override the default buckets (an override is clamped to `max_model_len` / `max_num_batched_tokens`: entries above the limit are dropped as unreachable, and the limit itself is appended when missing, so every schedulable length always has a bucket) — the KV buckets are geometric (powers of two from `block_size`) because the recorded set is a product of both axes, so dense buckets make warmup unaffordable at a long context; the query buckets are coarse, evenly-spaced ones for the same reason. Encoder attention on the rectangular path is traced *into* the block graph rather than run behind the opaque `unified_attention_with_output`, so inductor fuses it with the projections and the FFN. That costs one block compile per declared rectangle at warmup and needs a stick-aligned head size, so a `head_size=32` model (granite-30m, MiniLM) and any eager run fall back to the opaque path automatically — `install_encoder`'s predicate is the only gate, there is no switch. The **ragged path is never traced in**: it dispatches once per group, so the block graph's structure would depend on the step's group multiset, which cannot be enumerated at startup.
- **KV cache layout**: two decoder backends. Default is head-major (`SpyreHeadMajorAttentionBackend`, `[num_blocks, num_kv_heads, block_size, head_size]`), which drops the per-page permute the kernels do before the matmuls, paying for it in the KV write; `SPYRE_ATTN_KV_LAYOUT=token_major` selects `SpyreAttentionBackend` and its `[num_blocks, block_size, num_kv_heads, head_size]` page. Head-major always compiles attention (its 2-D page gather fails eager) and carries no ALiBi, so both of those need `token_major`. **Batched decode** (`SPYRE_BATCHED_DECODE`, default on) is the reverse: under the default tiled walk only head-major supplies the per-stick page index the kernel needs, so a token-major run keeps the per-sequence loop whatever the flag says. They share everything above the cache's memory and duplicate only the advertised shape, the allocation, and the three kernels that read a page. The head-major store is fragile in a specific way: it needs one index tensor per KV head (rows of a `[KV, T]` tensor are correct while a row fits one int32 stick and then silently corrupt every head past it, sometimes escalating to a device fault, so a short-token test passes while long prefill is wrong) and a materialized source (a single-token fused-QKV view already reports contiguous at a nonzero storage offset, so an in-graph `contiguous()` is a no-op; `clone()` does copy to offset zero but did not fix it either, so the store uses an elementwise pass).
