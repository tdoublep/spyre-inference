...
- **KV cache layout**: two decoder backends. Default is token-major (`[num_blocks, block_size, num_kv_heads, head_size]`); `SPYRE_ATTN_KV_LAYOUT=head_major` selects `SpyreHeadMajorAttentionBackend`, which stores `[num_blocks, num_kv_heads, block_size, head_size]` and so drops the per-page permute the kernels do before the matmuls, paying for it in the KV write. They share everything above the cache's memory and duplicate only the advertised shape, the allocation, and the three kernels that read a page. The head-major store is fragile in a specific way: it needs one index tensor per KV head (rows of a `[KV, T]` tensor silently store head 0's rows, then fault the device once `T` passes a stick) and a source that is a real offset-0 tensor (a single-token fused-QKV view is flagged contiguous at a nonzero storage offset, so an in-graph `contiguous()`/`clone()` of it is elided and the per-head slice reads the wrong data).
