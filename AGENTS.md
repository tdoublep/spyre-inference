...
- **Compilation**: default is compiled (`STOCK_TORCH_COMPILE`), not eager. Attention compiles in its own domain: `SpyreAttentionImpl` reads `get_current_vllm_config().compilation_config.mode` at construction, so building it outside a `set_current_vllm_config` context raises. Tests force `enforce_eager=True` where they want eager — don't assume "eager in tests." `STOCK_TORCH_COMPILE` compiles one transformer block at a time; `SPYRE_COMPILE_GRANULARITY=model` restores the whole-model graph. Attention kernels are recorded during warmup rather than compiled lazily in the serving path: warmup walks `SpyreAttnBucketer.variants()` and traces each one. `SPYRE_ATTN_RECORD=0` restores the lazy behaviour, and `SPYRE_ATTN_KV_BUCKETS` / `SPYRE_ATTN_QUERY_BUCKETS` override the default buckets (an override is clamped to `max_model_len` / `max_num_batched_tokens`: entries above the limit are dropped as unreachable, and the limit itself is appended when missing, so every schedulable length always has a bucket) — the KV buckets are geometric (powers of two from `block_size`) because the recorded set is a product of both axes, so dense buckets make warmup unaffordable at a long context; the query buckets are coarse, evenly-spaced ones for the same reason. One exception to "attention compiles in its own domain": a decode-only batch on the batched decode kernel has no per-sequence Python loop, so `attn_layer` traces that kernel *into* the block graph (`SPYRE_FUSED_DECODE_ATTN`, on by default wherever `SPYRE_BATCHED_DECODE` applies), saving a device-program boundary per block per step. Its shape parameters then become block-graph guards, so `_record_fused_decode_graphs` dummy-runs each reachable `(rows, num_seqs, num_blocks)` at warmup. Two constraints there are load-bearing, and both fail as `Incompatible host_size and dim_order` in torch-spyre layout propagation: the kernel must read the KV cache through the same slot-major view the scatter writes (a page-shaped alias of it becomes an AOT-autograd synthetic base, which does not lower), and the query must come from the staging buffer (rotary embedding leaves a rank-4 buffer, and a layout cannot be projected onto an argument of higher rank than the gather's output).
