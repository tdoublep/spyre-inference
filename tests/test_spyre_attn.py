...
def test_kv_cache_update_traced_by_caller(default_vllm_config, configure_device: str):
    """The production path: the scatter is traced into the caller's graph.

    Mutating the pages through a `view` of a graph input is only cheap if
    Inductor re-inplaces the scatter; a correct result here plus the absence of
    a fallback is what says the traced form is usable at all.
    """
    set_random_seed(0)
