...
def _to_cache_device(cache_cpu: torch.Tensor, device: torch.device) -> torch.Tensor:
    """Move a slot-major KV cache to ``device``, pinning the layout on Spyre.

    The reshape_and_cache scatter is only correct when the slot dim is outermost
    in the device layout, so tests must allocate the same way the model runner
    does. On CPU the layout argument does not apply.
    """
    ...
    return cache_cpu.to(
        ...
        device_layout=slot_major_kv_layout(
            num_slots, num_kv_heads, head_size, cache_cpu.dtype
        ),
    )
...
def ref_attn(
    ...
    for i in range(num_seqs):
        ...
        # Gather each page's slot range from the slot-major cache.
        # Each page: [block_size, num_kv_heads, head_size]
...
def test_reshape_and_cache_scatter(
    ...
    One index_copy_ replaces the previous host-driven loop over same-page slot
    runs, so the distribution of tokens across pages no longer changes the write
    strategy — the parametrization now just pins that every distribution lands
    correctly. Untouched slots must keep their sentinel, which is what catches
    the wrong-rows failure mode of an indirect store on a non-compliant layout.
    """
    set_random_seed(0)
