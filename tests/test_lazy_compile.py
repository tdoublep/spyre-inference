...
The decision uses only local signals -- ``torch.compiler.is_compiling()`` and the
compile mode sampled at construction -- so nothing here walks the model. Only the
last test traces, and tracing resolves the accelerator stream, which opens the
single contested Spyre card; hence its ``compile`` marker.
"""
...
def test_a_class_without_the_mixin_is_a_loud_error(compile_calls) -> None:
    """Opting in means mixing in CompileOutermost. Forgetting it must not silently
    fall back to eager -- that reads as "compiled" while quietly being slow."""
...
def test_the_artifact_does_not_become_a_submodule(compile_calls, mode) -> None:
    """A compiled callable in _modules would show up in named_parameters and
    state_dict, breaking weight save/reload."""
    mode(CompilationMode.STOCK_TORCH_COMPILE)
    norm = _Norm()
...
def test_parameter_names_are_untouched(compile_calls, mode) -> None:
    """torch.compile applied to the module (rather than one bound method) would
    reparent weights under _orig_mod and break reload_weights."""
    mode(CompilationMode.STOCK_TORCH_COMPILE)
    tail = _Tail()
...
def test_each_instance_compiles_its_own_kernel(compile_calls, mode) -> None:
    """The tail norm and the embedding are separate instances of separate classes;
    caching per class would hand one the other's graph."""
    mode(CompilationMode.STOCK_TORCH_COMPILE)
...
def test_an_enclosing_graph_absorbs_the_layer_without_breaking(mode) -> None:
    """The decisive case: a decorated layer inside a fullgraph block must be traced
    into the block's graph, not compiled again on its own."""
    mode(CompilationMode.STOCK_TORCH_COMPILE)
    ...
    with fresh_cache():
        ...
        # fullgraph=True would have raised on a graph break; assert the layer also
        # declined to build a second, redundant artifact.
