...
@pytest.mark.parametrize(
    ("running", "expected"),
    [
        # Nothing running: admit one prefill.
        ([], 1),
        # Mid-prefill: no slot left, no further admission.
        ([_request(512)], 1),
        # Prompt fully computed, so it decodes from here: admit the next prefill.
        ([_request(1984)], 2),
        # Decodes do not block an admission but do occupy running slots.
