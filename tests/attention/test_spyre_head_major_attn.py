...
The token-major suite in ``test_spyre_attn.py`` covers everything above the cache
layout, and both backends share that code, so this file exercises what the head-major
layout changes: the advertised shape, the allocation, the store's row index, and the
three kernels that read a page in the new shape.
"""
...
# The two fixtures below are per-module in this directory (test_spyre_attn.py and
# test_spyre_encoder_attn.py each carry their own copy); kept in step with those.
@pytest.fixture()
...
def _fresh_pages(
    ...
    """Zeroed head-major K/V pages, allocated the way the worker allocates them.

    On Spyre this goes through the impl's own ``allocate_pages`` so the test cache
    carries the production device layout; a host-populated cache with that layout
    pinned would be numerically wrong, so every write below goes through the impl.
    """
...
def _write(impl, kv_cache, key, value, slots, device) -> None:
    """One KV store through the impl, at the index its layout wants."""
...
def _run_head_major_attn_test(
    ...
    """Validate SpyreHeadMajorAttentionImpl against the same CPU reference.

    Unlike the token-major body this seeds the cache entirely through the impl's own
    store — history included — because the head-major device layout cannot be
    reproduced by a host-populated transfer.
    """
    ...
    # Head-major expectation, filled alongside the slot lists that seed the device.
...
# ---------------------------------------------------------------------------
# Layout arithmetic and the contract, no device needed
# ---------------------------------------------------------------------------
...
def test_head_major_scatter(
    ...
    Zero is the marker rather than a sentinel fill because the fill would itself have
    to go through this store, and a store covering the whole cache is a shape
...
# ---------------------------------------------------------------------------
# Cross-layout equivalence and the runner's allocation
# ---------------------------------------------------------------------------
...
def test_head_major_matches_token_major(
    ...
    # Widened onto the bucketer's padded block count: build() iterates every padded
    # block, so a table sized to the real count leaves the padding with no page to read.
    from vllm.config import get_current_vllm_config
