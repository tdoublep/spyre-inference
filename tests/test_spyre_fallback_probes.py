        ...
        pytest.param(
            ...
            marks=pytest.mark.xfail(
                ...
                reason=(
                    ...
                    "storage. SpyreAttentionImpl._online_softmax_attention no longer "
                    "trips this: its result is exactly query_len rows, so the "
                    "write-back source is never a prefix view. Anything that "
                    "reintroduces a padded result has to copy before the write. NB: "
                    "the overrun only appears once another prefix-view slice write at a "
