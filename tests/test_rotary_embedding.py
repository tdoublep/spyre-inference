...
def test_bound_rope_position_cache_truncates_device_cache(default_vllm_config):
    """bound_rope_position_cache shrinks the device rotation cache to the bound, and
    positions below it still match forward_native.

    The in-graph gather costs time proportional to the whole source cache, so a model
    whose max_model_len is far below max_position_embeddings must not pay for the rest.
    """
    from vllm.model_executor.layers.rotary_embedding.base import RotaryEmbedding
