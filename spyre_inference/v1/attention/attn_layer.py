...
class SlotMapping:
    ...
    def _write_index(self, slot_mapping: torch.Tensor, device: torch.device):
        """The device-side KV store index for this group's cache layout. The slot
        mapping itself for the token-major cache; the head-major one expands a slot
        into one index per head."""
