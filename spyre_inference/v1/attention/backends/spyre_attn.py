...
class SpyreAttentionImpl(AttentionImpl[SpyreAttentionMetadata]):
    ...
    def allocate_pages(
        ...
        Called by ``TorchSpyreModelRunner.initialize_kv_cache_tensors``, which owns
        neither the shape nor the device layout: both belong to the backend, and the
        head-major backend answers with its own.
        """
    ...
    def kv_write_index(self, slot_mapping: torch.Tensor, device: torch.device):
        """Mirror a host slot mapping to the KV store index ``do_kv_cache_update`` takes.

        Token-major puts a token's heads in one contiguous run, so the slot itself is
        the index; ``attn_layer.SlotMapping`` publishes whatever this returns.
        """
