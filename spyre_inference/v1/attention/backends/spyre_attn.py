...
def _create_compilable_page_attn(
    ...
    def specialized_paged_attn_kernel(
        ...
        if store_mode == "copy":
            ...
            return out
        if store_mode == "index":
            # copy_ cannot be used here: this sequence owns only part of `out`.
...
class SpyreAttentionImpl(AttentionImpl[SpyreAttentionMetadata]):
    ...
    def __init__(
        ...
        # Destination rows per (q_start, query_len), mirrored once and reused.
        self._out_row_tables: dict[tuple[int, int], torch.Tensor] = {}
        ...
        for seq_idx in range(num_seqs):
            ...
            if fused_store_ok:
                if output.shape[0] == query_len:
                    # This sequence owns every row; a whole-tensor copy_ needs no
                    # index and no relayout.
                    store_mode = "copy"
                elif output.shape[0] > 1:
                    # Partial ownership needs the indirect store. A single-row
                    # destination is excluded: it writes nothing (torch-spyre#4007).
                    store_mode = "index"
                    out_rows = self._out_row_tables.get((q_start, query_len))
                    if out_rows is None:
                        out_rows = torch.arange(
                            q_start, q_end, dtype=torch.int32, device=_target_device
                        )
                        self._out_row_tables[(q_start, query_len)] = out_rows
