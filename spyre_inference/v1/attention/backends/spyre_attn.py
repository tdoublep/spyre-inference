...
def _create_compilable_bucketed_decode_attn(
    ...
    Gathers one block at a time, as the single-sequence kernel does, so a page
    feeds the matmul directly instead of being staged in a whole-bucket buffer
    first.
    """
    ...
    def specialized_bucketed_decode_kernel(
        ...
        #   slice below does not emit a Mod(d0, ...) stick expression.
...
class SpyreAttentionMetadata(AttentionMetadata):
    ...
    # Spyre's index_copy_ requires int64. block_ids_padded rows are stick-padded
    # so the kernel's per-block row slice stays stick-aligned. mask_by_block is
    # pre-permuted for cheap axis-0 slicing in the dispatch.
    ...
    def build(
        ...
        if max_query_len == 1 and self.sliding_window is None and num_seqs >= _MIN_SEQS_BUCKET:
            ...
            if b_seqs is not None and b_blocks is not None:
                ...
                # One row per block, stick-padded: the kernel slices row i at
                # Dynamo-trace time, and an inner dim narrower than the stick
                # width (32) emits a Mod(d0, ...) stick expression the inductor
                # rejects.
                index_len = (
                ...
                block_ids_padded_cpu = torch.zeros(b_blocks, index_len, dtype=torch.int32)
                for s, n in enumerate(num_active):
                    n_use = min(n, b_blocks)
                    for b in range(n_use):
                        block_ids_padded_cpu[b, s] = block_table[s, b]
...
class SpyreAttentionImpl(AttentionImpl[SpyreAttentionMetadata]):
    ...
    def _run_bucketed_decode_dispatch(
        ...
        # slice emits no Mod(d0, ...) stick expression; (2) K/V/q fold (B_seqs, KV) into
