...
# When set, the KV-cache scatter is traced into the compiled transformer block
# (see v1/attention/attn_layer.py) instead of running as a second compiled graph
# launched from inside the opaque attention op. Set to 0 to restore the old path.
...
class SpyreAttentionMetadataBuilder(AttentionMetadataBuilder[SpyreAttentionMetadata]):
    ...
    def __init__(
        ...
        # Layers of this KV-cache group, for publishing the per-step device slot
        # mapping. The builder's own `device` is CPU, so the target device comes
    ...
    def _publish_slot_mapping(self, slot_mapping: torch.Tensor, num_actual_tokens: int) -> None:
        """Mirror this step's slot mapping to the device, once, before the graph runs.

        The attention layers read it as a plain tensor attribute, so no device
...
class SpyreAttentionBackend(AttentionBackend):
    ...
    # False routes the KV write out of the attention op; the Spyre attention
    # layer performs it via SpyreAttentionImpl.do_kv_cache_update.
...
class SpyreAttentionImpl(AttentionImpl[SpyreAttentionMetadata]):
    ...
    def __init__(
        ...
        # Slot-major views of this layer's pages; see kv_slot_views().
    ...
    def forward(
        ...
        if not KV_WRITE_IN_GRAPH and attn_metadata.slot_mapping_device is None:
        ...
        # Skipped when the layer already scattered them inside the outer graph.
    ...
    def kv_slot_views(self, kv_cache: SpyrePagedKVCache) -> SpyrePagedKVCache:
        """Slot-major [num_slots, num_kv_heads, head_size] views of the pages.

        Built once and cached: Inductor cannot lower a store through a view of a
        Spyre-layout tensor created inside a graph, so the traced scatter needs
        the views to already exist when the block is traced. Reshaping is valid
        because a view keeps the slot-outermost device layout.
        """
    ...
    def do_kv_cache_update(
        ...
        kv_cache: k/v pages, each [num_blocks, block_size, num_kv_heads, head_size]
        slot_mapping: [num_tokens] on the pages' device

        Returns the mutated slot-major K view. The caller hands it to the
        attention op so the scatter is ordered before the read.
        """
        ...
        if torch.compiler.is_compiling():
            # The caller's graph owns the scatter, so emit it as plain aten ops.
            k_slots.index_copy_(0, slot_mapping, key)
            v_slots.index_copy_(0, slot_mapping, value)
        else:
            # Eager caller: the scatter needs its own compiled artifact, since
            # eager index_copy_ rejects an int32 index and falls back to CPU
            # with an int64 one.
