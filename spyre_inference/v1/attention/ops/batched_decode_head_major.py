...
"""Batched multi-sequence decode over a head-major KV cache, behind ``SPYRE_BATCHED_DECODE``.

A copy of ``batched_decode.py`` whose only difference is the gathered page's shape:
the head-major cache already presents ``[entries, num_kv_heads, block_size, head_size]``,
so the token-major path's permute is gone.
"""
