...
A copy of ``page_attn.py`` whose only difference is the gathered page's shape: the
head-major cache already presents ``[num_kv_heads, block_size, head_size]``, so the
token-major path's permute is gone.
"""
