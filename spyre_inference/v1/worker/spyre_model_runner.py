...
class TorchSpyreModelRunner(GPUModelRunner):
    ...
    def initialize_kv_cache_tensors(self, kv_cache_config, kernel_block_sizes):
        ...
        Each layer gets its own SpyrePagedKVCache(k_pages, v_pages), a single tensor
        each in the shape and device layout its attention impl's `allocate_pages`
        chooses — token-major by default, head-major under
        SPYRE_ATTN_KV_LAYOUT=head_major — matching what that backend's
        ...
        for kv_cache_tensor in kv_cache_config.kv_cache_tensors:
            ...
            # The layout belongs to the backend, not here: ask the layer's impl for it.
            # A layer without one (fixture stubs) gets the token-major default.
