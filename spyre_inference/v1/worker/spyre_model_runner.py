...
class _SpyreModelWrapper:
    ...
    def _arm_traced_attention(self) -> None:
        """Publish this step's attention inputs to the layers: a traced body cannot
        mirror anything to device itself."""
...
class TorchSpyreModelRunner(GPUModelRunner):
    ...
    def _fuse_attn_graph(self) -> bool:
        ...
        Compiled only: traced eagerly, the KV scatter would be an eager index_copy_,
        which falls back to CPU.

        Known wrong for batches of more than one sequence — tokens come out shifted
        by one position.
        """
