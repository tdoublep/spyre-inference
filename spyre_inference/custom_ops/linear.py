...
def spyre_linear_t(x: torch.Tensor, weight_t: torch.Tensor, bias: torch.Tensor | None):
    ...
    with the head axis still outermost (see `SpyreAttentionImpl.decode_attention`).
    Folding it to `[tokens, num_heads * head_size]` first would make the matmul
    contract more than one axis, which the backend scheduler rejects; contracting each
    head against its own slice of `Wᵀ` and summing is the same sum in a different
    order. The `view` is free — `Wᵀ` is `[in, out]` contiguous, so its head slices are
    already its rows.
    """
