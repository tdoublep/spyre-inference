...
class TorchSpyrePlatform(CpuPlatform):
    ...
    def _cap_offset_position_max_model_len(cls, vllm_config: VllmConfig) -> None:
        ...
        514-row table), not the ``max_position_embeddings`` vLLM derives. Dense pooling
        pads every sequence to the declared length, so a length of 514 makes the pad
        rows alone index two past the table on every request.
        """
