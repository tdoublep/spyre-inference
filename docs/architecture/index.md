...
|---|---|---|---|
| `RMSNorm` | `SpyreRMSNorm` | Spyre | `forward_oot` runs a `compile_when_outermost` kernel directly on Spyre. Deliberately **all float16, no float32 promotion**: upstream's `forward_native` (now `vllm.ir.ops.rms_norm`) reduces in float32, which makes the `[num_tokens, 1]` variance a float32 staggered-EA value. A Spyre pointwise can only mix that with a float16 buffer when the float16 side broadcasts along the stick, so once inductor fuses the norm into the float16 pointwise that feeds it — what every decoder block does — the graph is rejected above 33 tokens |
