...
`_patched_attention_forward`) and puts the scatter, the page loop and the softmax in the
block's graph — one graph per block, with the KV write inside it. The host-side work the
untraced body does per layer per step (query padding, the permutes, the output copy) is an
eager dispatch each, and that is what the traced body removes.

It pays because attention hands its result to `o_proj` with the head axis still outermost,
which drops a relayout copy and a launch per layer.
